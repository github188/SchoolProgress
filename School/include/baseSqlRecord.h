...
class Record : public EntryManager<EntryName,EntryNone<2>,EntryNone<3>,true>
{
	public:
		...
		void put(const char *filedName,T value)
		{
			...
			if(!entry)
			{
				Test::logger->debug("Record::put error the entry is NULL");
				return;
			}
		...
		void put(const char *filedName,const void *data,DWORD len)
		{
			...
			if(!entry)
			{
				Test::logger->debug("Record::put void error the entry is NULL");
				return;
			}
			strncpy(entry->name,filedName,strlen(filedName));
			...
			if(!addFiled(entry))
			{
				Test::logger->debug("addFile error filedName:%s",entry->name);
				DELETE(entry);
			}
		}
		template <typename T>
		void putk(const char *filedName,T value,DWORD key)
		{
			...
			if(!entry)
			{
				Test::logger->debug("Record::putk error entry is NULL");
				return;
			}
			strncpy(entry->name,filedName,strlen(filedName));
			entry->id = key;
		...
		void putk(const char *filedName,const void *data,DWORD len,DWORD key)
		{
			...
			if(!entry)
			{
				Test::logger->debug("Record::putk void error entry is NULL");
				return;
			}
			strncpy(entry->name,filedName,strlen(filedName));
			entry->id = key;
		...
		Filed* get(const char *filedName)
		{
			...
			std::transform(filedName,&filedName[strlen(filedName)],temp,Test::upper);
			Filed *entry = NULL;
			rwlock.rdlock();
