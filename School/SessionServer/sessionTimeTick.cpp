...
void SessionTimeTick::run()
{
	...
	while(!isFinal())
	{
		...
		if( m_oneSec(SessionTimeTick::s_currentTime) )
		{
			...
			else if( m_tenSec(SessionTimeTick::s_currentTime) )
			{
				tenSec();
			}
			else if( m_oneMin(SessionTimeTick::s_currentTime) )
			{
				oneMin();
			}
			else if( m_fiveMin(SessionTimeTick::s_currentTime) )
			{
				fiveMin();
			}
			else if( m_oneQuarter(SessionTimeTick::s_currentTime) )
			{
				oneQuarter();
			}
			else if( m_oneClocker(SessionTimeTick::s_currentTime) )
			{
				oneHour();
			}
			else if( m_zeroClocker(SessionTimeTick::s_currentTime) )
			{
				zeroHour();
			}
			else if( m_fourClocker(SessionTimeTick::s_currentTime) )
			{
				fourHour();
			}
		}
